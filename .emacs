...
;; Use 'alien' indexing with fd for speed (70k+ files in repo)
...
;; org-weekly — multi-file task sync into a weekly plan
;; Install: cd ~/.emacs.d/site-lisp && git clone https://github.com/tedmellors/org-weekly.git org-weekly
(let ((org-weekly-dir (expand-file-name "site-lisp/org-weekly" user-emacs-directory)))
  (if (file-directory-p org-weekly-dir)
      (progn
        (add-to-list 'load-path org-weekly-dir)
        (require 'org-weekly)
        (with-eval-after-load 'org
          (define-key org-mode-map (kbd "C-c s") #'org-weekly-sync-next)))
    (message "org-weekly not installed — run: git clone https://github.com/tedmellors/org-weekly.git %s"
             org-weekly-dir)))
